...
async def search_view(request: Request, q: str) -> SelectSearchResponse:
    ...
    if r.status_code == 404:
        ...
        for co in data:
            regions[co["region"]].append(
                {"value": co["cca3"], "label": co["name"]["common"]}
            )
        options = [{"label": k, "options": v} for k, v in regions.items()]
    return SelectSearchResponse(options=options)  # type: ignore[arg-type]
    ...
    match kind:
        case "login":
            return [
                ...
                c.ModelForm(
                    model=LoginForm, display_mode="page", submit_url="/api/forms/login"
                ),
            ]
        case "select":
            return [
        ...
        case "big":
            return [
                ...
                c.ModelForm(
                    model=BigModel, display_mode="page", submit_url="/api/forms/big"
                ),
            ]
        case _:
...
class SelectForm(BaseModel):
    ...
    search_select_single: str = Field(
        json_schema_extra={"search_url": "/api/forms/search"}
    )
    search_select_multiple: list[str] = Field(
        json_schema_extra={"search_url": "/api/forms/search"}
    )
...
class BigModel(BaseModel):
    ...
    info: Annotated[str | None, Textarea(rows=5)] = Field(
        None, description="Optional free text information about you."
    )
    repo: str = Field(
        json_schema_extra={"placeholder": "{org}/{repo}"}, title="GitHub repository"
    )
    profile_pic: Annotated[UploadFile, FormFile(accept="image/*", max_size=16_000)] = (
        Field(description="Upload a profile picture, must not be more than 16kb")
    )
    profile_pics: Annotated[list[UploadFile], FormFile(accept="image/*")] | None = (
        Field(None, description="Upload multiple images")
    )
    dob: date = Field(
        title="Date of Birth",
